...
/**
 * https://zhuanlan.zhihu.com/p/571738567
 * http1.1
 * 1. 同一域名最多6、7个并发连接（浏览器防止服务器压力过大的限制），队头阻塞的连接阻塞，其他连接继续跑
 * 
 * http2
 * 1. 1.x中采用文本传输，2采用2进制传输（有多种文本形式，要使用就必须考虑多种场景，二进制则方便健壮）
 * 2. 采用多路复用，一个tcp连接可以有无数个请求
 * 请求被分成stream，每个stream由二进制帧组成
 * 请求过程中管道内发送不同流的帧数，实现并发。服务器通过流和帧来拼接一个完整的请求
 * 多条流，对端可以通过帧标识知道属于哪个请求。这样就可以避免队头阻塞，极大提高性能（帧代表最小的数据单位，每个帧会标识出属于哪个流，流由多个帧组成）
 * 3.对头部压缩，通过客户端和服务器维护相同哈希表，key是索引，value是表头字段，如果表头字段出现过，则只传索引
